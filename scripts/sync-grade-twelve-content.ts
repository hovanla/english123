...
async function main() {
  const grade = await prisma.grade.findUnique({ where: { slug: "lop-12" } });
  if (!grade) throw new Error("Không tìm thấy cấp Lớp 12. Hãy chạy local:setup trước.");
  const baseCourse = await prisma.course.upsert({
    where: { gradeId_slug: { gradeId: grade.id, slug: "tieng-anh-lop-12" } },
    create: {
      gradeId: grade.id, slug: "tieng-anh-lop-12", title: "Tiếng Anh Lớp 12",
      description: "16 bài Lớp 12: nhìn tranh đoán từ, nghe mẫu câu và phản xạ trong tình huống đời thực.",
  ...
  let activityCount = 0;
  for (const [unitIndex, seed] of gradeTwelveUnits.entries()) {
    const existingUnit = course.units.find((item) => item.slug === seed.slug);
    const unitData = { title: seed.title, theme: seed.theme, description: seed.description, imageUrl: gradeTwelveBoardUrl(seed.slug), order: unitIndex + 1, status: ContentStatus.PUBLISHED };
    ...
    const boardUrl = gradeTwelveBoardUrl(seed.slug);
    await prisma.asset.upsert({
      where: { url: boardUrl },
      create: { url: boardUrl, pathname: boardUrl.replace(/^\//, ""), contentType: "image/webp", altText: `Bộ tranh Lớp 12 chủ đề ${seed.theme}`, source: "English123 · OpenAI image generation", license: AssetLicense.OWNED, width: 1250, height: 1250 },
      update: { pathname: boardUrl.replace(/^\//, ""), altText: `Bộ tranh Lớp 12 chủ đề ${seed.theme}`, source: "English123 · OpenAI image generation", license: AssetLicense.OWNED, width: 1250, height: 1250 },
    });
    const lessonSeeds = buildGradeTwelveLessons(seed);
  ...
  console.log(`Đã đồng bộ ${gradeTwelveUnits.length} bài Lớp 12 với ${activityCount} hoạt động.`);
}
