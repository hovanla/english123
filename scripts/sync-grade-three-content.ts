...
async function main() {
  const course = await prisma.course.findFirst({
    where: { grade: { slug: "lop-3" } },
    include: { units: { include: { lessons: { include: { activities: { orderBy: { order: "asc" } } } } } } },
  });
  if (!course) throw new Error("Không tìm thấy khóa Tiếng Anh Lớp 3. Hãy chạy local:setup trước.");

  await prisma.course.update({
    where: { id: course.id },
    data: {
      title: "Tiếng Anh Lớp 3",
      description: "20 unit Lớp 3: nhìn tranh đoán từ, nghe mẫu câu và phản xạ trong tình huống đời thực.",
  ...
  let activityCount = 0;
  for (const [unitIndex, seed] of gradeThreeUnits.entries()) {
    const existingUnit = course.units.find((item) => item.slug === seed.slug);
    const unitData = {
      title: seed.title,
      theme: seed.theme,
      description: seed.description,
      imageUrl: gradeThreeBoardUrl(seed.slug),
      order: unitIndex + 1,
      status: ContentStatus.PUBLISHED,
    };
    ...
    const boardUrl = gradeThreeBoardUrl(seed.slug);
    await prisma.asset.upsert({
      ...
      create: {
        ...
        altText: `Bộ tranh Lớp 3 chủ đề ${seed.theme}`,
    ...
    const lessonSeeds = buildGradeThreeLessons(seed);
    ...
    for (const [lessonIndex, lessonSeed] of lessonSeeds.entries()) {
      ...
      const lessonData = {
        ...
        order: lessonIndex + 1,
        estimatedMinutes: lessonSeed.slug === "tu-vung" ? 9 : 5,
        status: ContentStatus.PUBLISHED,
      };
  ...
  console.log(`Đã đồng bộ ${gradeThreeUnits.length} unit Lớp 3 với ${activityCount} hoạt động.`);
}
