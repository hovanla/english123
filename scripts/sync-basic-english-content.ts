...
async function main() {
  const grade = await prisma.grade.upsert({
    ...
    create: {
      ...
      description: "Học từ vựng, mẫu câu và ngữ pháp nền tảng theo từng bước.",
      order: 14,
    },
    update: {
      name: "Tiếng Anh cơ bản",
  ...
  const baseCourse = await prisma.course.upsert({
    where: { gradeId_slug: { gradeId: grade.id, slug: "tieng-anh-co-ban-1" } },
    create: {
      gradeId: grade.id,
  ...
  let activityCount = 0;
  for (const [unitIndex, seed] of basicEnglishUnits.entries()) {
    const existingUnit = course.units.find((item) => item.slug === seed.slug);
    const imageUrl = basicEnglishBoardUrl(seed.slug);
    const unitData = {
      title: seed.title,
      theme: seed.theme,
    ...
    await prisma.asset.upsert({
      ...
      create: {
        ...
        altText: `Bộ tranh tiếng Anh cơ bản chủ đề ${seed.theme}`,
    ...
    const lessonSeeds = buildBasicEnglishLessons(seed);
    ...
    for (const [lessonIndex, lessonSeed] of lessonSeeds.entries()) {
      ...
      const lessonData = {
        ...
        order: lessonIndex + 1,
        estimatedMinutes: lessonIndex === 0 ? 10 : lessonIndex === 1 ? 6 : 3,
        status: ContentStatus.PUBLISHED,
      };
  ...
  console.log(`Đã đồng bộ ${basicEnglishUnits.length} chặng với ${activityCount} hoạt động.`);
}
