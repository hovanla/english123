...
async function main() {
  const course = await prisma.course.findFirst({
    where: { grade: { slug: "lop-5" } },
    include: { units: { include: { lessons: { include: { activities: { orderBy: { order: "asc" } } } } } } },
  });
  if (!course) throw new Error("Không tìm thấy khóa Tiếng Anh Lớp 5. Hãy chạy local:setup trước.");
  await prisma.course.update({
    where: { id: course.id },
    data: { title: "Tiếng Anh Lớp 5", description: "20 unit Lớp 5: nhìn tranh đoán từ, nghe mẫu câu và phản xạ trong tình huống đời thực.", status: ContentStatus.PUBLISHED },
  });

  const activeUnitIds: string[] = [];
  let activityCount = 0;
  for (const [unitIndex, seed] of gradeFiveUnits.entries()) {
    const existingUnit = course.units.find((item) => item.slug === seed.slug);
    const unitData = { title: seed.title, theme: seed.theme, description: seed.description, imageUrl: gradeFiveBoardUrl(seed.slug), order: unitIndex + 1, status: ContentStatus.PUBLISHED };
    ...
    const boardUrl = gradeFiveBoardUrl(seed.slug);
    await prisma.asset.upsert({
      where: { url: boardUrl },
      create: { url: boardUrl, pathname: boardUrl.replace(/^\//, ""), contentType: "image/webp", altText: `Bộ tranh Lớp 5 chủ đề ${seed.theme}`, source: "English123 · OpenAI image generation", license: AssetLicense.OWNED, width: 1536, height: 1024 },
      update: { pathname: boardUrl.replace(/^\//, ""), altText: `Bộ tranh Lớp 5 chủ đề ${seed.theme}`, source: "English123 · OpenAI image generation", license: AssetLicense.OWNED, width: 1536, height: 1024 },
    });

    const lessonSeeds = buildGradeFiveLessons(seed);
  ...
  console.log(`Đã đồng bộ ${gradeFiveUnits.length} unit Lớp 5 với ${activityCount} hoạt động.`);
}
