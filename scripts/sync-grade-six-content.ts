...
async function main() {
  const grade = await prisma.grade.findUnique({ where: { slug: "lop-6" } });
  if (!grade) throw new Error("Không tìm thấy cấp Lớp 6. Hãy chạy local:setup trước.");
  const baseCourse = await prisma.course.upsert({
    where: { gradeId_slug: { gradeId: grade.id, slug: "tieng-anh-lop-6" } },
    create: {
      gradeId: grade.id, slug: "tieng-anh-lop-6", title: "Tiếng Anh Lớp 6",
      description: "16 bài Lớp 6: nhìn tranh đoán từ, nghe mẫu câu và phản xạ trong tình huống đời thực.",
  ...
  let activityCount = 0;
  for (const [unitIndex, seed] of gradeSixUnits.entries()) {
    const existingUnit = course.units.find((item) => item.slug === seed.slug);
    const unitData = { title: seed.title, theme: seed.theme, description: seed.description, imageUrl: gradeSixBoardUrl(seed.slug), order: unitIndex + 1, status: ContentStatus.PUBLISHED };
    ...
    const boardUrl = gradeSixBoardUrl(seed.slug);
    await prisma.asset.upsert({
      where: { url: boardUrl },
      create: { url: boardUrl, pathname: boardUrl.replace(/^\//, ""), contentType: "image/webp", altText: `Bộ tranh Lớp 6 chủ đề ${seed.theme}`, source: "English123 · OpenAI image generation", license: AssetLicense.OWNED, width: 1536, height: 1024 },
      update: { pathname: boardUrl.replace(/^\//, ""), altText: `Bộ tranh Lớp 6 chủ đề ${seed.theme}`, source: "English123 · OpenAI image generation", license: AssetLicense.OWNED, width: 1536, height: 1024 },
    });
    const lessonSeeds = buildGradeSixLessons(seed);
    ...
    for (const [lessonIndex, lessonSeed] of lessonSeeds.entries()) {
      ...
      const lessonData = { title: lessonSeed.title, description: lessonSeed.description, order: lessonIndex + 1, estimatedMinutes: lessonSeed.slug === "tu-vung" ? 11 : 5, status: ContentStatus.PUBLISHED };
  ...
  console.log(`Đã đồng bộ ${gradeSixUnits.length} bài Lớp 6 với ${activityCount} hoạt động.`);
}
