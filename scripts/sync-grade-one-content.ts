...
async function main() {
  const course = await prisma.course.findFirst({
    where: { grade: { slug: "lop-1" } },
    include: { units: { include: { lessons: { include: { activities: { orderBy: { order: "asc" } } } } } } },
  });
  if (!course) throw new Error("Không tìm thấy khóa Tiếng Anh Lớp 1. Hãy chạy local:setup trước.");

  await prisma.course.update({
    where: { id: course.id },
    data: { title: "Tiếng Anh Lớp 1", description: "20 unit Lớp 1: nhìn tranh đoán từ, nghe mẫu câu và phản xạ trong tình huống gần gũi.", status: ContentStatus.PUBLISHED },
  });

  const activeUnitIds: string[] = [];
  let activityCount = 0;
  for (const [unitIndex, seed] of gradeOneUnits.entries()) {
    let unit = course.units.find((item) => item.slug === seed.slug);
    if (!unit) {
      unit = await prisma.unit.create({
        data: { courseId: course.id, slug: seed.slug, title: seed.title, theme: seed.theme, description: seed.description, imageUrl: gradeOneBoardUrl(seed.slug), order: unitIndex + 1, status: ContentStatus.PUBLISHED },
        include: { lessons: { include: { activities: { orderBy: { order: "asc" } } } } },
      });
    } else {
      unit = await prisma.unit.update({
        where: { id: unit.id },
        data: { title: seed.title, theme: seed.theme, description: seed.description, imageUrl: gradeOneBoardUrl(seed.slug), order: unitIndex + 1, status: ContentStatus.PUBLISHED },
        include: { lessons: { include: { activities: { orderBy: { order: "asc" } } } } },
      });
    }
    activeUnitIds.push(unit.id);

    const boardUrl = gradeOneBoardUrl(seed.slug);
    await prisma.asset.upsert({
      where: { url: boardUrl },
      create: { url: boardUrl, pathname: boardUrl.replace(/^\//, ""), contentType: "image/webp", altText: `Bộ tranh Lớp 1 chủ đề ${seed.theme}`, source: "English123 · OpenAI image generation", license: AssetLicense.OWNED, width: 1536, height: 1536 },
      update: { pathname: boardUrl.replace(/^\//, ""), altText: `Bộ tranh Lớp 1 chủ đề ${seed.theme}`, source: "English123 · OpenAI image generation", license: AssetLicense.OWNED, width: 1536, height: 1536 },
    });

    const lessonSeeds = buildGradeOneLessons(seed);
    ...
    for (const [lessonIndex, lessonSeed] of lessonSeeds.entries()) {
      ...
      if (!lesson) {
        lesson = await prisma.lesson.create({
          data: { unitId: unit.id, slug: lessonSeed.slug, title: lessonSeed.title, description: lessonSeed.description, order: lessonIndex + 1, estimatedMinutes: lessonSeed.slug === "tu-vung" ? 6 : 5, status: ContentStatus.PUBLISHED },
          include: { activities: true },
        });
      } else {
        lesson = await prisma.lesson.update({
          where: { id: lesson.id },
          data: { title: lessonSeed.title, description: lessonSeed.description, order: lessonIndex + 1, estimatedMinutes: lessonSeed.slug === "tu-vung" ? 6 : 5, status: ContentStatus.PUBLISHED },
          include: { activities: true },
        });
      }
  ...
  console.log(`Đã đồng bộ ${gradeOneUnits.length} unit Lớp 1 với ${activityCount} hoạt động.`);
}
