...
async function main() {
  const grade = await prisma.grade.upsert({
    where: { slug: "tieng-anh-co-ban" },
    create: { slug: "tieng-anh-co-ban", name: "Tiếng Anh người lớn", level: "Từ nền tảng đến nâng cao", description: "Lộ trình giao tiếp và ngữ pháp cho người học trưởng thành.", order: 14 },
    update: { name: "Tiếng Anh người lớn", level: "Từ nền tảng đến nâng cao", description: "Lộ trình giao tiếp và ngữ pháp cho người học trưởng thành." },
  });
  const baseCourse = await prisma.course.upsert({
    where: { gradeId_slug: { gradeId: grade.id, slug: "tieng-anh-nang-cao" } },
    create: { gradeId: grade.id, slug: "tieng-anh-nang-cao", title: "Tiếng Anh nâng cao", description: "12 chủ đề nâng cao, ba chặng ôn tập và phản xạ lập luận trong tình huống thực tế.", order: 3, status: ContentStatus.PUBLISHED },
    update: { title: "Tiếng Anh nâng cao", description: "12 chủ đề nâng cao, ba chặng ôn tập và phản xạ lập luận trong tình huống thực tế.", order: 3, status: ContentStatus.PUBLISHED },
  });
  ...
  for (const [unitIndex, seed] of advancedEnglishUnits.entries()) {
    const imageUrl = advancedEnglishBoardUrl(seed.slug);
    ...
    await prisma.asset.upsert({
      where: { url: imageUrl },
      create: { url: imageUrl, pathname: imageUrl.replace(/^\//, ""), contentType: "image/svg+xml", altText: `Bộ tranh tiếng Anh nâng cao chủ đề ${seed.theme}`, source: "English123 · minh họa SVG tự biên soạn", license: AssetLicense.OWNED, width: 1500, height: 900 },
      update: { pathname: imageUrl.replace(/^\//, ""), altText: `Bộ tranh tiếng Anh nâng cao chủ đề ${seed.theme}`, source: "English123 · minh họa SVG tự biên soạn", license: AssetLicense.OWNED, width: 1500, height: 900 },
    });

    const lessonSeeds = buildAdvancedEnglishLessons(seed);
    ...
    for (const [lessonIndex, lessonSeed] of lessonSeeds.entries()) {
      ...
      const lessonData = { title: lessonSeed.title, description: lessonSeed.description, order: lessonIndex + 1, estimatedMinutes: lessonIndex === 0 ? 10 : lessonIndex === 1 ? 7 : 4, status: ContentStatus.PUBLISHED };
  ...
  console.log(`Đã đồng bộ ${advancedEnglishUnits.length} chặng với ${activityCount} hoạt động.`);
}
