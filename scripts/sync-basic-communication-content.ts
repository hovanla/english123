...
async function main() {
  const grade = await prisma.grade.upsert({
    ...
    create: { slug: "giao-tiep-mat-goc", name: "Giao tiếp tiếng Anh", level: "Người mới bắt đầu", description: "Phản xạ nghe và nói theo tình huống đời thực.", order: 13 },
    update: { name: "Giao tiếp tiếng Anh", description: "Phản xạ nghe và nói theo tình huống đời thực." },
  });
  const baseCourse = await prisma.course.upsert({
    where: { gradeId_slug: { gradeId: grade.id, slug: "giao-tiep-co-ban" } },
    create: { gradeId: grade.id, slug: "giao-tiep-co-ban", title: "Giao tiếp cơ bản", description: "23 chủ đề nghe–nói đời thực, hai chặng ôn tập và phản xạ theo hội thoại.", order: 2, status: ContentStatus.PUBLISHED },
    update: { title: "Giao tiếp cơ bản", description: "23 chủ đề nghe–nói đời thực, hai chặng ôn tập và phản xạ theo hội thoại.", order: 2, status: ContentStatus.PUBLISHED },
  });
  ...
  for (const [unitIndex, seed] of basicCommunicationUnits.entries()) {
    const imageUrl = basicCommunicationBoardUrl(seed.slug);
    const imageContentType = imageUrl.endsWith(".png") ? "image/png" : "image/svg+xml";
    ...
    await prisma.asset.upsert({
      where: { url: imageUrl },
      create: { url: imageUrl, pathname: imageUrl.replace(/^\//, ""), contentType: imageContentType, altText: `Bộ tranh giao tiếp cơ bản chủ đề ${seed.theme}`, source: "English123 · minh họa tình huống tự biên soạn", license: AssetLicense.OWNED, width: 1500, height: 900 },
      update: { pathname: imageUrl.replace(/^\//, ""), altText: `Bộ tranh giao tiếp cơ bản chủ đề ${seed.theme}`, source: "English123 · minh họa SVG tự biên soạn", license: AssetLicense.OWNED, width: 1500, height: 900 },
    });

    const lessonSeeds = buildBasicCommunicationLessons(seed);
    ...
    for (const [lessonIndex, lessonSeed] of lessonSeeds.entries()) {
      ...
      const lessonData = { title: lessonSeed.title, description: lessonSeed.description, order: lessonIndex + 1, estimatedMinutes: lessonIndex === 0 ? 9 : lessonIndex === 1 ? 7 : 3, status: ContentStatus.PUBLISHED };
  ...
  console.log(`Đã đồng bộ ${basicCommunicationUnits.length} chặng với ${activityCount} hoạt động.`);
}
