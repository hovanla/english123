...
describe("grade eleven curriculum", () => {
  ...
  it("builds complete valid image-led lessons", () => {
    for (const unit of gradeElevenUnits) {
      ...
      expect(lessons[1].activities.every((activity, index) => activity.type === "SENTENCE" && activity.payload.mode === "RESPONSE_RECALL" && activity.payload.imageUrl === gradeElevenBoardUrl(unit.slug) && activity.payload.spriteIndex === unit.words.length + index && activity.payload.imageHint)).toBe(true);
